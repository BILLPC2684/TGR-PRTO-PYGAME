...
typedef struct {
 ...
 uint8_t  *MEM,*ROMPG[33],Page[2],RGB[3],id,id2;
 ...
 */
 while(1) {
  ...
  if (valread > 0) {
   ...
   } else if (strstr(command, "tick"  )!=NULL) { printf("tock"); break; }
   //send(client, buffer, strlen(buffer), 0); //Echoing
  if(sys.Running[sys.id]) {
   uint8_t A   =  sys.MEM[(sys.IP[sys.id]+1)%0xD800000] >> 4 ;       //4 \.
   uint8_t B   =  sys.MEM[(sys.IP[sys.id]+1)%0xD800000] & 0xF;       //4 |-> A/B/C = 1.5 bytes
   uint8_t C   =  sys.MEM[(sys.IP[sys.id]+2)%0xD800000] >> 4 ;       //4 /'
   int32_t IMM = (sys.MEM[(sys.IP[sys.id]+2)%0xD800000] & 0xF) << 8; //4 \.
   IMM  = (IMM |  sys.MEM[(sys.IP[sys.id]+3)%0xD800000])<< 8 ;       //8 |->  IMM  = 3.5 bytes
   IMM  = (IMM |  sys.MEM[(sys.IP[sys.id]+4)%0xD800000])<< 8 ;       //8 |
   IMM |=         sys.MEM[(sys.IP[sys.id]+5)%0xD800000];             //8 /'
   if (sys.Debug == true) {
    printf("\nIC: 0x%08x   \t>> [",sys.IP[sys.id]);
    for(int i=0;i<6;i++){printf("0x%02x",sys.MEM[(sys.IP[sys.id]+i)%0xD800000]);if(i<5){printf(", ");}}
    printf("]  [A:%2i, B:%2i, C:%2i, IMM:0x%07x] | [A:%c, B:%c, C:%c]\n\\REGs: [",A,B,C,IMM,REGchar[A],REGchar[B],REGchar[C]);
    for(int i=0;i<8;i++){printf("%c:0x%04x",REGchar[i],sys.REG[sys.id][i]);if(i<7){printf(", ");}}
    printf("] | TotalRan: %li\n\\StackPointer: 0x%07x/%li | StackBase: 0x%07x/%li\n\\\\StackData:[",sys.TIPS,sys.SP,sys.SP,sys.SPMAX[sys.id],sys.SPMAX[sys.id]);
//    for (int i = sys.SPMAX[sys.id]; i >= sys.SPMIN[sys.id]; --i){ if((i+1)%2==0) { printf(" 0x"); } printf("%02x",sys.MEM[i]); if(i%16==0 && i != 0) { printf("\n"); } }
    printf("]\n \\instruction: "); //,sys.REG[sys.id][0],sys.REG[sys.id][1],sys.REG[sys.id][2],sys.REG[sys.id][3],sys.REG[sys.id][4],sys.REG[sys.id][5],sys.REG[sys.id][6],sys.REG[sys.id][7]);
   }
 
   for(sys.id=0;sys.id<2;sys.id++) {
    switch(sys.MEM[sys.IP[sys.id]%0xD800000]) { //* Instuctions
     case 0x00: //mov     A, B/IMM              - moves B/IMM, to A
     ...
     case 0x0E: //led     {A,B,C}/{IMM}         - sets the System's LED color | IMM is #RRGGBB
      ...
      break;
     case 0x0F: //cmpeq   A, B                  - if A == B then continue, else skip 1 instruction
      if (sys.Debug == true) { printf("CMP=\n"); }
      
      break;
     case 0x10: //cmplt   A, B                  - if A <  B then continue, else skip 1 instruction
      if (sys.Debug == true) { printf("CMP<\n"); }
      
      break;
     case 0x11: //cmpgt   A, B                  - if A  > B then continue, else skip 1 instruction
      if (sys.Debug == true) { printf("CMP>\n"); }
      
      break;
     case 0x12: //wmem    {B,C},   A            - Writes to A to MEM[B..C]
      if (sys.Debug == true) { printf("WMEM\n"); }
      
      break;
     case 0x13: //rmem    {B,C},   A            - Reads from MEM[B..C] to A
      if (sys.Debug == true) { printf("RMEM\n"); }
     ...
     case 0xFF: //nop                           - what you expected me to do something? NOPe!
      ...
      break;
     default: printf("[EMU Service] CPU#%i: Unknown Operation 0x%2x",sys.id,sys.MEM[sys.IP[sys.id]]); break;
    }
   }
  }
